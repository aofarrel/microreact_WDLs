...
workflow Microreact_Remove_Teammates {
	input {
		File token
		String team_uri
...
task mr_kick_teammate {
	...
	command <<<
		...
		payload = json.dumps(json_ish)
		if verbose: print(payload)
		
		def kick_teammates(token, payload, retries=-1):
			if retries < ~{max_python_retries}:
				try:
					response = requests.post("https://microreact.org/api/teams/remove-member",
					...
					retries =+ 1
					wait(retries)
					kick_teammates(token, payload, retries)
		...
		kick_teammates(TOKEN_STR, payload)
