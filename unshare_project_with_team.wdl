...
task mr_unshare_with_team {
	...
	command <<<
		...
		def rm_mr_project_from_team(token, payload, retries=-1):
			if retries < ~{max_python_retries}:
				try:
					...
					if response.status_code == 200:
						users = response.json()
						return users
					print(f"Failed to unshare project ~{project_uri} from ~{team_uri} [code {response.status_code}]: {response.text}")
					retries =+ 1
					wait(retries)
					rm_mr_project_from_team(token, payload, retries)
				except Exception as e: # ignore: broad-exception-caught
					print(f"Caught exception trying to unshare project ~{project_uri} from team ~{team_uri}: {e}")
					retries =+ 1
					wait(retries)
					rm_mr_project_from_team(token, payload, retries)
			else:
				print(f"Failed to unshare project ~{project_uri} from team ~{team_uri} after ~{max_python_retries} retries. Something's broken.")
				exit(1)
